...
func NewIPModifer(template *Template, config *Config, path string) *IPModifer {
	return &IPModifer{
		TemplateObj: template,
		ConfigObj:   config,
		Path:        path,
...
func (m *IPModifer) output(dirpath string) bool {
	...
	for component, ipSet := range m.ConfigObj.Components {
		for i, _ := range ipSet {
			...
			err := ioutil.WriteFile(path.Join(m.Path, component+strconv.Itoa(i)+"_cf.yml"), out, 0777)
			if err != nil {
